...
This module provides comprehensive regression analysis for macroeconomic data,
including linear and regularized regression models to understand the relationships
between economic variables and GDP growth.
...
class MacroeconomicRegression:
    """
    A comprehensive class for macroeconomic regression analysis.
    
    This class provides methods to:
    - Load and preprocess macroeconomic data
    - Train multiple regression models (Linear, Ridge, Lasso, ElasticNet)
    - Evaluate model performance
    - Visualize results and coefficients
    - Export results for further analysis
    """
    
    def __init__(self, data_path='data/external/global_merged_all.csv'):
        """
        Initialize the MacroeconomicRegression class.
        ...
        self.feature_mapping = {
            'investment_GDP': 'Gross_capital_formation_percent_GDP',
            'exports_GDP': 'Exports_wb',
            'inflation': 'US_Inflation_Index',
            'population': 'Population_wb',
            'oil_price': 'Oil_Price_Brent',
    ...
    def preprocess_data(self, start_year=1970, end_year=2021, min_years_per_country=10):
        ...
        """
        print("Preprocessing data for regression analysis...")
    ...
    def _engineer_features(self, df):
        ...
        # Create investment/GDP ratio
        if 'Gross_capital_formation_percent_GDP' in df.columns:
            df['investment_GDP'] = df['Gross_capital_formation_percent_GDP']
        else:
            # Calculate from available data
            df['investment_GDP'] = (df['Gross_capital_formation_main'] / df['GDP_main']) * 100
        
        # Create exports/GDP ratio
        if 'Exports_wb' in df.columns and 'GDP_usd' in df.columns:
            df['exports_GDP'] = (df['Exports_wb'] / df['GDP_usd']) * 100
        else:
            df['exports_GDP'] = (df['Exports_main'] / df['GDP_main']) * 100
        
        # Use US inflation as proxy for global inflation (can be improved with country-specific data)
        df['inflation'] = df['US_Inflation_Index']
        
        # Population (log transform for better scaling)
        df['population'] = np.log(df['Population_wb'] + 1)
        
        # Oil price (already in good scale)
        df['oil_price'] = df['Oil_Price_Brent']
        
        # Federal funds rate
        df['fed_rate'] = df['Federal_Funds_Rate']
        
        # Add diversification index (simplified HHI)
    ...
    def plot_coefficients(self, figsize=(12, 8)):
        ...
        plt.title('Regression Coefficients Comparison', fontsize=16, fontweight='bold')
    ...
    def plot_predictions(self, model_name='LinearRegression', figsize=(10, 6)):
        ...
        plt.title(f'{model_name}: Actual vs Predicted GDP Growth', fontweight='bold')
    ...
    def export_results(self, output_dir='results/'):
        ...
        # Export coefficients
        coef_df = self.get_coefficients()
        coef_df.to_csv(f'{output_dir}model_coefficients.csv')
        ...
        for name, result in self.results.items():
            ...
            pred_df.to_csv(f'{output_dir}{name.lower()}_predictions.csv', index=False)
    ...
    def run_full_analysis(self, start_year=1970, end_year=2021, 
        ...
        """
        print("=" * 60)
        print("MACROECONOMIC REGRESSION ANALYSIS")
        print("=" * 60)
        
        # Load and preprocess data
        X, y = self.preprocess_data(start_year, end_year)
